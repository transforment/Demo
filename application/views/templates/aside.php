<div class="navmenu navbar-default sidebar navmenu-fixed-left offcanvas-sm">
    ...
    <ul class="nav navmenu-nav" id="side-menu">
        ...
        <li class="sidebar-search">
            <form role="search" method="post"action="<?php echo base_url();?>Xem_ho_so">
            <div class="input-group custom-search-form">
        ...
        if((isset($_SESSION['name_user']))&&
            ...
            echo '<li><a href="'.base_url('admin/ho_so_da_tao').'">
                  <i class="fa fa-plus fa-fw"></i> Hồ sơ đã nhận <span class="badge right">'.$this->db->where('status',0)->where('ma_so_can_bo_thu_tien',($_SESSION['ma_can_bo']))->count_all_results('ho_so').'</span></a></li>';
        }
        if((isset($_SESSION['name_user']))&&
            (($_SESSION['level']==21)||($_SESSION['level']==22))){
            echo '<li><a href="'.base_url('admin/admin_phong_ban').'">
                  <i class="fa fa-plus fa-fw"></i> Hồ sơ xử lý <span class="badge right">'.$this->db->where('status',1)->where('ma_so_can_bo_thu_tien',($_SESSION['ma_can_bo']))->count_all_results('ho_so').'</span></a></li>';
        ...
        if((isset($_SESSION['name_user']))&&
            ...
            echo '<li><a href="'.base_url('admin/admin_tra_ho_so').'">
                  <i class="fa fa-plus fa-fw"></i> Hồ sơ trả dân <span class="badge right">'.$this->db->where('status',3)->or_where('status',4)->or_where('status',6)->or_where('status',7)->where('ma_so_can_bo_thu_tien',($_SESSION['ma_can_bo']))->count_all_results('ho_so').'</span></a></li>';
            echo '<li><a href="'.base_url('admin/thong_ke').'">
                  <i class="fa fa-bar-chart-o  fa-fw"></i> Thống kê </a></li>';
        }
        if((isset($_SESSION['name_user']))&&
