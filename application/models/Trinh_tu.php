<?php
class trinh_tu extends CI_Model {
	public $id;
	public $noi_dung;
	...
	public function array_trans($val){
		...
		$this->db->where('id', $val);
   		$q = $this->db->get('trinh_tu')->row();
		$data= $q->noi_dung;
	
	    return $data;
	}
	public function is_match($ten){
    	$query = $this->db->get('trinh_tu');
...
if($noi_dung=='')return ;
    	$array=array(
			...
			$this->db->insert('trinh_tu',$array);
			
			$this->db->select_max('id');
			$query = $this->db->get('trinh_tu')->result_array();
...
if($noi_dung=='')return ;
			$this->db->where('noi_dung', $noi_dung);
   			$node_map = $this->db->get('trinh_tu')->row();
