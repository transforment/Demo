<?php
class ket_qua extends CI_Model {
	public $id;
	public $noi_dung;
	...
	public function array_trans($val){
		...
		$this->db->where('id', $val);
   		$q = $this->db->get('ket_qua')->row();
		$data= $q->noi_dung;
	
	    return $data;
	}
	public function is_match($ten){
    	$query = $this->db->get('ket_qua');
...
if($noi_dung=='')return ;
    	$array=array(
			...
			$this->db->insert('ket_qua',$array);
			
			$this->db->select_max('id');
			$query = $this->db->get('ket_qua')->result_array();
...
if($noi_dung=='')return ;
			$this->db->where('noi_dung', $noi_dung);
   			$node_map = $this->db->get('ket_qua')->row();
